...
class PublicKey:
    ...
    "__repr__ returns the string representation of of the object"
    def __repr__(self):
...
class SecretKey:
    ...
    def verify(self, message, signature):
        ...
        #signatureRead = open("signatureC.txt", "r")
        #sig = [int(x) for x in signatureRead.read().split()]
        #signature = bytes(sig)
        #print("signature: ", signature)
        
        #pubkeyRead = open("pubkeyC.txt", "r")
        #self.h = [int(x) for x in pubkeyRead.read().split()]

        #hashedRead = open("hashedC.txt", "r")
        #hashed = [int(x) for x in hashedRead.read().split()]
        
        #messageRead = open("messageC.txt", "r")
        #mssg = [int(x) for x in messageRead.read().split()]
        #message = bytes(mssg) 
        #var = b'jS3AAe,ZB#{pzq0%$*l+kBQ54O{9p+s{\n'.decode('utf-8')
        #print("mssg type: ", type(message))
        ...
        hashed = self.hash_to_point(message, salt) #gives wrong hashed value, function or salt?
